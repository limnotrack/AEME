...
- [`plot_est_wbal()`](plot_est_wbal.md) : Plot estimated water balance
  diagnostics from an aeme object.
- [`plot_flows()`](plot_flows.md) : Plot inflows and/or outflows
- [`plot_fluxes()`](plot_fluxes.md) : Plot fluxes
- [`plot_glm_diagnostics()`](plot_glm_diagnostics.md) : Plot GLM-AED
  diagnostic variables to debug unrealistic oxygen values
...
- [`plot_wbal_summaries()`](plot_wbal_summaries.md) : Plot Water Balance
  Summaries
- [`plot_weir_calibration()`](plot_weir_calibration.md) : Visualise
  calibrated weir parameters (C, h_inv) from calc_water_balance().
...
- [`read_model_hypsograph()`](read_model_hypsograph.md) : Load model
  hypsograph from configuration
