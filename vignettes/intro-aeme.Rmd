...
The AEME package hosts four one-dimensional hydrodynamic models: the DYnamic
REservoir Simulation Model (DYRESM), the General Lake Model (GLM), the
General Ocean Turbulence Model (GOTM, which has been adapted for closed basins
for application to lakes and reservoirs), and Simstrat. The models can be
coupled to their corresponding water quality models, the DYRESM-CAEDYM
(Computational Aquatic Ecosystem Dynamics Model), GLM-AED (Aquatic Ecosystem
Dynamics Model), GOTM-WET (Water Ecosystem Tool), and Simstrat-AED2/AED.

This vignette assumes at least one model is already installed -- see
`vignette("installing-models")` if `build_aeme()` below reports a missing
executable.
...
-   **stop** - End date of the simulation (character). The end date must be in
...
using the function \code{assess_aeme()} and also to calibrate the model using
the [aemetools](https://github.com/limnotrack/aemetools) package.

The `lake` observations are stored in a data frame with the following required
columns:
...
-   **var_aeme** - Variable name of the observation (character). The variable
    names and input preparation are described in the [AEME inputs
...
integrated sample, when the observation covers a depth interval) and **sd**
(the measurement standard deviation, in the variable's units, used for
observation weighting during calibration). The legacy `depth_from` /
`depth_to` column pair is still accepted and is collapsed to a single `depth`
(interval midpoint) with a deprecation warning.
...
-   **value** - Value of the observation (numeric). The value must be in metres
    above sea level and within the range of the hypsograph elevations.
...
-   elevation - named list of elevations at which the outflows occur. This is 
    important that it falls within the elevation range in the hypsograph. Set to 
    `-1` if the outflow is at the surface.
...
-   file - Either the name of the file e.g. "glm3.nml" for model specific
    files (whichever of `glm3.nml`/`glm4.nml` is actually on disk is resolved
    automatically -- see `find_glm_nml()` -- so this table entry always uses
    the literal string `"glm3.nml"` regardless of the GLM version an object
    was built with) or "met" for meteorological variables or "inf" for inflow
    ...
    reflect the current outflows slot soon...).
...
-   index - Index of the parameter. This is only used for GLM-AED parameters
    that are vectors e.g. sediment parameters ("sed_temp_mean").
