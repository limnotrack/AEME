...
should have a similar file structure to the following:
...
aeme
```
The Aeme object can also be downloaded using the `aemetools::get_aeme()`
function which will download the AEME object for a given lake ID. This is a 
programmatic way to access the AEME objects for the lakes in the LERNZmp platform,
however, it requires an API key which can be obtained by contacting the LERNZmp
team at [data@limnotrack.com](mailto:data@limnotrack.com).


```{r}
#| label: lernzmp-download-aeme
#| eval: FALSE
