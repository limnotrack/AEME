...
expand_met <- function(met, lat, lon, elev, print.plot = FALSE) {
  ...
  if (!is.prsttn) {
    ...
    prsttn <- get_station_pressure(prmslp = prmslp, elevation = elev,
                                   tmpair = tmpair) 
  ...
  if (!is.prmslp) {
    ...
    # estimate station pressure from sea level pressure
    prmslp <- get_mean_sea_level_pressure(prsttn = prsttn, elevation = elev, 
                                          tmpair = tmpair)
  }
...
#' Estimate station pressure at elevation
#' @noRd
estimate_station_pressure <- function(
    tmpair,
    humrel,
    elev,
    sea_level_pressure_hPa = 1013.25
  ...
  es <- 6.112 * exp((17.67 * tmpair) / (tmpair + 243.5))
  
  # Actual vapor pressure (hPa)
  e <- (humrel / 100) * es
  
  # Mixing ratio (kg/kg)
  w <- 0.622 * e / (sea_level_pressure_hPa - e)
  
  # Virtual temperature (K)
  Tv <- T_K * (1 + (Rv / Rd - 1) * w)
  
  # Hypsometric equation
  P <- sea_level_pressure_hPa * exp(-g * elev / (Rd * Tv))
