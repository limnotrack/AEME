...
expand_met <- function(met, coords.xyz, print.plot = FALSE) {
  ...
  for (i in c("radswd","tmpair","pprain")) {
    ...
    if(isFALSE(get(paste0("is.",i)))) { stop(paste0(i, " is required but not present"))}
  }
  ...
  if (!is.humrel) {
    ...
    if (!is.tmpdew) { stop("humrel (RH %) is not supplied, therefore, tmpdew (dewpoint temperature) is required, but not present")}
    
    tmpdew <- met[,which(grepl("tmpdew", colnames(met)))]
    
    # estimate humidity from wet bulb temperature, see: https://www.omnicalculator.com/physics/relative-humidity
  ...
  if (!is.tmpdew) {
    ...
    if(!is.humrel) { stop("tmpdew (dewpoint temperature) is not supplied, therefore, humrel (RH %) is required, but not present")}
  ...
  if (!is.prsttn) {
    ...
    if (!is.prmslp) { stop("prsttn (station level pressure hPa) is not supplied, therefore, prmslp (mean sea level pressure hPa) is required, but not present")}
    
    prmslp <- met[, which(grepl("prmslp",colnames(met)))]
    
    # estimate station pressure from sea level pressure
    prsttn <- psychrolib::GetStationPressure(prmslp * 100, coords.xyz[3], tmpair)/100
  ...
  if (!is.prmslp) {
    ...
    # estimate station pressure from sea level pressure
    prmslp <- psychrolib::GetSeaLevelPressure(prsttn * 100, coords.xyz[3], tmpair)/100
  ...
  if (is.wnduvu & is.wnduvu) {
    ...
    if(!is.wnddir) { 
      ...
      # adjust wind direction to represent direction to (not from)
      wnddir <- met[,which(grepl("wnddir",colnames(met)))] %>% ifelse((. + 180) > 360, . - 180, . +180)
    }
    
    if(!is.wndspd) { stop("wnduvu & wnduvv wind vector components not supplied, therefore, wndspd (& ideally direction) are required, but not present")}
