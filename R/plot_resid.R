...
plot_resid <- function(aeme, model, var_sim = "HYD_temp") {
  ...
  plist[["doy_resid"]] <- ggplot2::ggplot() +
  ...
  if (dep_chk) {
    plist[["resid_depth"]] <- ggplot2::ggplot() +
  ...
  plist[["resid_dist"]] <- ggplot2::ggplot() +
    ggplot2::geom_vline(xintercept = 0, linetype = "dashed") +
  ...
  pall <- patchwork::wrap_plots(plist, guides = "collect")
