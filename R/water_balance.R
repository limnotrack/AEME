...
water_balance <- function(hyps, inf, outf = NULL, 
  ...
  if (is.null(obs_lvl)) {
    ...
    if (nrow(obs_lvl) < 2) {
      ...
      ampl <- ((quantile(obs_lvl$value,0.9) - quantile(obs_lvl$value,0.1)) / 2) |> as.numeric()
      offset <- -1
      surf <- max(hyps[,1])
      ...
      ampl <- ((quantile(obs_lvl$value,0.9) - 
                  quantile(obs_lvl$value,0.1)) / 2) |> 
        as.numeric()
      offset <- -1
      surf <- median(obs_lvl$value)
      mod.lvl <- data.frame(Date = obs_met$Date) |> 
        dplyr::left_join(obs_lvl, by = "Date") |> 
        dplyr::mutate(yday = lubridate::yday(Date))
      
      fit <- stats::loess(value ~ yday, data = mod.lvl)
      newdata <- mod.lvl |> 
  ...
  # if(sum(!is.na(evap[["value"]])) < 10) {
  #   message("Estimating temperature using air temperature...")
  #   evap$value <- 5.0 + 0.75 * evap$MET_tmpair # (Stefan & Preud'homme, 2007) www.doi.org/10.1111/j.1752-1688.1993.tb01502.x
  # }
  ...
  evap <- obs_met |>
    ...
    dplyr::mutate(lvlwtr2 = mod_lvl(Date, surf = max(hyps[,1]), ampl = ampl,
                            offset = offset)) |>
    dplyr::mutate(area   = get_hyps_val(lvlwtr, hyps = hyps.ext),
...
# function to calculate actual surface area or volume
get_hyps_val <- function(lvlwtr, hyps) {
  
  lapply(lvlwtr, function(l) approx(hyps[,1], hyps[,2], xout = l)$y) |>
    unlist()
}
...
calc_V2 <- function(depth, elev, area, h = 0.1) {
  sapply(depth, \(d) {
...
# function for a simple sinusoidal water level
mod_lvl <- function(dates, surf, ampl, offset) {
  daysinyear <- ifelse(as.numeric(year(dates) %% 4 == 0), 366, 365)
