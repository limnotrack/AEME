...
read_model_nc <- function(aeme, model, vars_sim, nlev, 
  ...
  if (model == "dy_cd") {
    out_file <- file.path(lake_dir, model, "DYsim.nc")
  } else {
    out_file <- file.path(lake_dir, model, "output", "output.nc")
  }

  
  if (!file.exists(out_file)) {
    cli::cli_alert_warning("No file {.file {out_file}} present.")
    return(NULL)
  }
  
  nc <- ncdf4::nc_open(out_file, return_on_error = TRUE)
  if (nc$error) {
    cli::cli_abort("Could not open netCDF file: {.file {out_file}}")
  }
  on.exit({
    ncdf4::nc_close(nc)
  })
  ...
  vapply(1:ncol(var), \(c) {
    ...
    if (is.na(depth[c])) {
      ...
      if (length(unique(depths[!is.na(depths[, c]), c])) == 1) {
        ...
        non_na <- !is.na(var[, c])
        if (sum(non_na) <= 1) return(rep(NA, nlev))
