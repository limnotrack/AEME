  ...
  psi_func <- function(iflag, ZoL) {
    ...
    if (ZoL < 0) {
      ...
      psic <- 1.5 * log((1/3) * (1 + chic + chic^3)) -
        sqrt(3) * atan((1 + 2 * chic) / sqrt(3)) + pi / sqrt(3)
      Fw <- 1 / (1 + ZoL^2)
...
#       ri <- g * zw * (delt + 0.61 * ta_k * delq) / (ta_k * delw0^2)
#       
#       
#       if (ri <= 0.25) {
#         Wstar_old <- Wstar
...
#           ZQoL <- zq * oL
#           
#           wpsi <- psi_gotm(1, ZWoL)
#           tpsi <- psi_gotm(2, ZToL)
#           qpsi <- psi_gotm(2, ZQoL)
#           
#           ZoW <- 0.011 * Wstar^2 / g + 0.11 * vis_air / (Wstar)
...
#           ZoQ <- rq * cff
#           
#           denomT <- log(zt / (ZoT)) - tpsi
#           denomQ <- log(zq / (ZoQ)) - qpsi
...
#           
#           bf <- -g / ta_k * Wstar * TVstar
#           wgus <- if (bf > 0) beta * (bf * Zabl)^(1/3) else 0
#           delw <- sqrt(w^2 + wgus^2)
#           
#           # after computing Wstar, Tstar, Qstar, wgus, delw
#           if (rr >= 0 && rr < 1000 &&
#               is.finite(Wstar) && is.finite(Qstar) && Wstar > 0) {
#             
#             Wstar_ok <- Wstar
#             Tstar_ok <- Tstar
#             Qstar_ok <- Qstar
...
#           }
#           
#           if (rr < 0 || rr >= 1000 || !is.finite(Wstar) || Wstar <= 0) {
#             ier <- -2
#             break
#           }
#           
...
#             (1 + 1.61 * qa) * Wstar * Tstar / ta_k
#           qe <- qe - rhoa * L * upvel * qa
#           
#           evap <- if (rain_impact && calc_evaporation)
#             rhoa / rho_0 * Wstar * Qstar else 0
#           
#         } else {
#           evap <- 0
#           qe   <- 0
...
psi_gotm <- function(iflag, ZoL) {
  ...
  if (ZoL < 0.0) {
    ...
    if (iflag == 1) {
      psik <- 2.0 * log(0.5 * (1.0 + chik)) +
        log(0.5 * (1.0 + chik * chik)) -
