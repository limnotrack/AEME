...
#'     plural).
#'
#' @return The `aeme` object with `time(aeme)$output_time_step` updated. The
#'   value must be greater than or equal to `time(aeme)$time_step`; [set_time()]
#'   raises `aeme_error_output_time_step` otherwise.
#' @export
#'
#' @seealso [set_time()]
#'
...
#' time(aeme)$output_time_step
set_output_time_step <- function(aeme, frequency) {
  secs <- .duration_to_seconds(frequency)
  set_time(aeme, output_time_step = secs)
}
